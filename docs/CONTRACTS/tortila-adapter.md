...
**Current state (Phase 0–2):** No auth on the journal.
The journal has no auth middleware. Any process that can reach `:8080` can read all endpoints.

**Required before production WTC deployment:**
- Add an API token to the journal (env `JOURNAL_READ_TOKEN`).
...
**Network boundary:** Until a reverse proxy or firewall rule is added, the journal is
accessible from `0.0.0.0:8080` — effectively public if the server's security group allows
port 8080. The WTC deployment checklist must include: restrict journal port to WTC server
IP only via security group / iptables rule before `BOT_ADAPTER_MODE=read-only` is used.
...
| `/api/overview` | GET | Full dashboard bundle | `getMetrics()` |
...
| `/api/marks` | GET | Live mark prices for open positions | Open position display |
| `/api/metrics/advanced` | GET | Sharpe/Sortino/Calmar/period returns | Advanced metrics |
| `/api/symbol_breakdown` | GET | Per-symbol P&L | Symbol breakdown chart |
| `/api/activity` | GET | Mixed event timeline | Activity/safety feed |
| `/api/drawdown` | GET | Underwater curve | DD chart |
| `/api/monthly` | GET | Monthly P&L | Monthly bar chart |
| `/api/calendar` | GET | Daily PnL heatmap | Calendar view |
| `/api/distribution` | GET | Trade PnL histogram | Distribution chart |
...
- The marks endpoint (`/api/marks`) has an internal 30s memory cache to avoid hammering BingX.
- WTC worker must not poll faster than every 30 seconds for marks.
- WTC worker must not poll `/api/overview` faster than every 60 seconds.
...
| `/api/overview` | 60 seconds |
| `/api/marks` | 60 seconds (respects journal's 30s cache) |
| `/api/trades/list` | 5 minutes |
| `/api/metrics/advanced` | 5 minutes |
...
| Marks endpoint (calls BingX) | 20 seconds |
...
**Phase gate:** `BOT_ADAPTER_MODE=read-only` must not be set in any environment until:
...
- API token auth is configured on the journal (`JOURNAL_READ_TOKEN`)
- Journal port `:8080` is firewall-restricted to WTC server IP only
...
All tests must pass (Vitest) before setting `BOT_ADAPTER_MODE=read-only` in any environment.
...
| `marks — BingX down` | Verify adapter handles empty marks gracefully, returns null unrealized PnL |
...
| API token auth on journal | P0 (before prod) | Add `Authorization: Bearer` token validation middleware |
