...
- Worker service: Tortila snapshot/import code exists and now has a local safe one-shot smoke command plus an admin heartbeat surface. The one-shot smoke command routes retained child output through the Phase 3.54 redactor, but a separately managed preview/production worker process still needs operator-approved deployment/monitoring if DB-first journal imports should refresh automatically.
