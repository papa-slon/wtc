 ...
 */
export async function loadLegacyOverviewPayload(): Promise<LegacyOverviewPayload> {
 ...
 */
export async function loadLegacyLiveOverview(): Promise<LegacyLiveOverview> {
  const opts = botAdapterOptions();
  const baseUrl = opts.legacyJournalUrl ?? '';
  const configured = opts.mode !== 'mock' && Boolean(baseUrl) && Boolean(opts.legacyReadToken);
