...
const markerPath = join(process.cwd(), '.next-e2e-auth-db', 'auth-db-e2e-prepared.json');
...
export function assertThrowawayDbName(databaseUrl: string): string {
  ...
  if (!/^wtc_test(?:_[a-z0-9]+)*$/.test(name)) {
    throw new Error(
      `Auth DB e2e refused: database "${name || '(none)'}" is not a throwaway test DB. ` +
        'Use a fresh database named wtc_test or wtc_test_auth_<suffix> only.',
    );
  }
  return name;
}
...
async function main(): Promise<void> {
  if (!url) throw new Error('Set AUTH_E2E_DATABASE_URL to a fresh throwaway Postgres database before running auth DB e2e.');
  if (!prepToken) throw new Error('Use npm run e2e:auth:db so the auth DB e2e prep token is generated.');
  ...
  try {
    ...
    if (existing.length > 0) {
      throw new Error(
        `Auth DB e2e refused: ${dbName} is not empty (${existing.length} table(s)). ` +
    ...
    if (files.length === 0) throw new Error('Auth DB e2e refused: no migration SQL files found.');
    ...
    console.log(`Prepared ${dbName} for auth DB e2e with ${files.length} migration(s) and demo seed data.`);
  } finally {
